...
class Forge:
    ...
    async def forge_object(  # pylint: disable=too-many-return-statements
        self,
        ...
        response = await self._make_api_call(url, action, object_json, session)
    ...
    async def forge_project(
        ...
        """
        uri = await self.forge_object(refined_object, session)
        if uri is None:
            raise ForgeError(
        ...
        if project_parameters:
            ...
            _ = self.forge_object(refined_parameters, session)

        return uri

    async def forge_experiment(
        self,
    ...
    async def forge_datafile(
        self, refined_object: Datafile, session: aiohttp.ClientSession
    ) -> None:
        ...
        _ = await self.forge_object(refined_object, session)
